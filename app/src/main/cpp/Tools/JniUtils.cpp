...
const char* JniUtils::JavaClassNameObjectArray = "[Ljava/lang/Object;";
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    Log::I(JniUtils::TAG, "Robot %s, vercode=%d", __PRETTY_FUNCTION__, VERSION_CODE);
...
{
    jclass jclass_bool = GetJavaClass(JavaClassNameBoolean);
...
{
    jclass jclass_int = GetJavaClass(JavaClassNameInteger);
...
{
    jclass jclass_double = GetJavaClass(JavaClassNameDouble);
...
std::shared_ptr<_jobject> JniUtils::GetLocalObjectRefSafely(JNIEnv* env, jobject value)
{
    std::shared_ptr<_jobject> ret;
    std::thread::id thread_id = std::this_thread::get_id();
...
std::shared_ptr<_jstring> JniUtils::GetStringSafely(JNIEnv* env, const char* str)
{
...
std::shared_ptr<_jobjectArray> JniUtils::GetObjectArraySafely(JNIEnv* env, jobject data[], int len)
{
    ...
    auto creater = [=]() -> jobjectArray {
        ENSURE_RUNON_THREAD(thread_id);
        jclass jclass_obj = GetJavaClass(JavaClassNameObject);
...
jclass JniUtils::GetJavaClass(const char* class_name)
{
    // CRASH if not found
